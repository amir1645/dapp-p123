...
const translations = {
  fa: {
    ...
    withdrawPool: "برداشت از Pool",
    withdrawSpecials: "برداشت از Special Pool",
    buyMinerTokens: "خرید توکن‌های ماینر",
    contributeMinerPool: "مشارکت در Miner Pool",
    successWithdraw: "موفقیت: برداشت انجام شد",
    successBuyMiner: "موفقیت: توکن‌های ماینر خریداری شد",
    successContribute: "موفقیت: مشارکت در Miner Pool انجام شد",
    errorWithdraw: "خطا: ",
    errorBuyMiner: "خطا: ",
    errorContribute: "خطا: ",
    insufficientBalance: "موجودی کافی نیست. حداقل نیاز به هزینه گس است.",
    enterAmount: "لطفاً مقدار را وارد کنید",
    switchNetwork: "لطفاً شبکه را به Polygon Mainnet تغییر دهید.",
    errorSwitch: "خطا در تغییر شبکه: "
  },
  ar: {
    title: "تطبيق PToken",
    ...
    withdrawPool: "سحب من Pool",
    withdrawSpecials: "سحب من Special Pool",
    buyMinerTokens: "شراء توكنات الماينر",
    contributeMinerPool: "المساهمة في Miner Pool",
    successWithdraw: "نجاح: تم السحب",
    successBuyMiner: "نجاح: تم شراء توكنات الماينر",
    successContribute: "نجاح: تمت المساهمة في Miner Pool",
    errorWithdraw: "خطأ: ",
    errorBuyMiner: "خطأ: ",
    errorContribute: "خطأ: ",
    insufficientBalance: "الرصيد غير كافٍ",
    enterAmount: "يرجى إدخال المبلغ",
    switchNetwork: "يرجى تغيير الشبكة إلى Polygon Mainnet.",
    errorSwitch: "خطأ في تغيير الشبكة: "
  },
  en: {
    title: "PToken DApp",
    ...
    withdrawSpecials: "Withdraw from Special Pool",
    buyMinerTokens: "Buy Miner Tokens",
    contributeMinerPool: "Contribute to Miner Pool",
    successWithdraw: "Success: Withdrawal completed",
    successBuyMiner: "Success: Miner tokens purchased",
    successContribute: "Success: Contributed to Miner Pool",
    errorWithdraw: "Error: ",
    errorBuyMiner: "Error: ",
    errorContribute: "Error: ",
    insufficientBalance: "Insufficient balance",
    enterAmount: "Please enter an amount",
    switchNetwork: "Please switch to Polygon Mainnet.",
    errorSwitch: "Error switching network: "
  }
};
...
function updateTexts() {
  ...
  document.getElementById("withdraw-pool-btn").textContent = t.withdrawPool;
  document.getElementById("withdraw-specials-btn").textContent = t.withdrawSpecials;
  document.getElementById("buy-miner-btn").textContent = t.buyMinerTokens;
  document.getElementById("contribute-miner-btn").textContent = t.contributeMinerPool;
}
...
async function connectWallet() {
  ...
  try {
    ...
    // چک کردن و سوئیچ شبکه به Polygon
    const network = await provider.getNetwork();
    const polygonChainId = 137;
    const polygonChainIdHex = '0x89';
    if (network.chainId !== polygonChainId) {
      try {
        await provider.send("wallet_switchEthereumChain", [{ chainId: polygonChainIdHex }]);
      } catch (switchError) {
        if (switchError.code === 4902) {
          try {
            ...
            document.getElementById("message").innerText = translations[currentLanguage].errorSwitch + addError.message;
            return;
          }
        } else {
          document.getElementById("message").innerText = translations[currentLanguage].errorSwitch + switchError.message;
          return;
        }
      }
      // چک دوباره شبکه بعد از سوئیچ
      const updatedNetwork = await provider.getNetwork();
      if (updatedNetwork.chainId !== polygonChainId) {
...
async function withdrawPool() {
  ...
  try {
    ...
    if (balance.lt(estimatedGasCost)) {
      document.getElementById("message").innerText = translations[currentLanguage].insufficientBalance;
      return;
    }

    const isWithdrawable = await contract.isPoolWithdrawable();
    if (!isWithdrawable) {
      document.getElementById("message").innerText = "برداشت از Pool در حال حاضر ممکن نیست.";
      return;
    }
    ...
    await tx.wait();
    document.getElementById("message").innerText = translations[currentLanguage].successWithdraw;
    await fetchUserInfo();
  } catch (err) {
    document.getElementById("message").innerText = translations[currentLanguage].errorWithdraw + (err.reason || err.message || "تراکنش رد شد");
  }
}

// تابع برداشت از Special Pool
...
async function buyMinerTokens() {
  ...
  try {
    ...
    const lastBuyTime = await contract.lastMinerBuyTime();
    const buyInterval = await contract.MINER_BUY_INTERVAL();
    const currentTime = Math.floor(Date.now() / 1000);
    if (currentTime < Number(lastBuyTime) + Number(buyInterval)) {
      document.getElementById("message").innerText = "لطفاً تا زمان بعدی خرید صبر کنید.";
      return;
    }
    ...
    await tx.wait();
    document.getElementById("message").innerText = translations[currentLanguage].successBuyMiner;
    await fetchUserInfo();
  } catch (err) {
    document.getElementById("message").innerText = translations[currentLanguage].errorBuyMiner + (err.reason || err.message || "تراکنش رد شد");
  }
}

// تابع مشارکت در Miner Pool
async function contributeToMinerPool() {
  if (!contract || !userAccount) return;

  const amount = document.getElementById("miner-amount").value;
  if (!amount || isNaN(amount) || amount <= 0) {
    document.getElementById("message").innerText = translations[currentLanguage].enterAmount;
    return;
  }
  ...
  try {
    ...
    const gasLimit = 2000000;
    const estimatedGasCost = gasPrice.mul(gasLimit);
    const totalRequired = contributionAmount.add(estimatedGasCost);
    ...
    await tx.wait();
    document.getElementById("message").innerText = translations[currentLanguage].successContribute;
    await fetchUserInfo();
  } catch (err) {
    document.getElementById("message").innerText = translations[currentLanguage].errorContribute + (err.reason || err.message || "تراکنش رد شد");
  }
}
