...
function detectWalletApp() {
  ...
  isInWalletApp = userAgent.includes("metamask") || userAgent.includes("coinbase") || userAgent.includes("trust");
  console.log("Wallet app detected:", isInWalletApp);
}
...
const translations = {
  fa: {
    ...
    errorRegister: "خطا در ثبت‌نام: ",
    errorUserInfo: "خطا در گرفتن اطلاعات: ",
    errorTree: "خطا در نمایش درخت: ",
    installWallet: "لطفاً کیف پول (مثل MetaMask) را نصب کنید یا از مرورگر داخلی کیف پول استفاده کنید!",
    walletNotDetected: "کیف پول شناسایی نشد. لطفاً مطمئن شوید که افزونه یا اپلیکیشن کیف پول نصب است.",
    invalidUpline: "خطا: کد آپلاین نامعتبر است",
    noTree: "درختی برای نمایش وجود ندارد.",
    switchNetwork: "لطفاً شبکه را به Polygon Mainnet تغییر دهید.",
    walletConnectFailed: "اتصال به WalletConnect ناموفق بود. لطفاً دوباره تلاش کنید."
  },
  ar: {
    title: "تطبيق PToken",
    ...
    errorConnection: "خطأ في التوصيل: ",
    errorRegister: "خطأ في التسجيل: ",
    errorUserInfo: "خطأ في جلب المعلومات: ",
    errorTree: "خطأ في عرض الشجرة: ",
    installWallet: "يرجى تثبيت محفظة (مثل MetaMask)!",
    walletNotDetected: "لم يتم اكتشاف المحفظة. يرجى التأكد من تثبيت إضافة أو تطبيق المحفظة.",
    invalidUpline: "خطأ: عنوان الراعي غير صالح",
    noTree: "لا يوجد شجرة للعرض.",
    switchNetwork: "يرجى تبديل الشبكة إلى Polygon Mainnet.",
    walletConnectFailed: "فشل الاتصال بـ WalletConnect. يرجى المحاولة مرة أخرى."
  },
  en: {
    title: "PToken DApp",
    ...
    installWallet: "Please install a wallet (e.g., MetaMask)!",
    walletNotDetected: "Wallet not detected. Please ensure the wallet extension or app is installed.",
    invalidUpline: "Error: Upline code is invalid",
    noTree: "No tree to display.",
    switchNetwork: "Please switch to Polygon Mainnet.",
    walletConnectFailed: "Failed to connect to WalletConnect. Please try again."
  }
};

let currentLanguage = 'fa';

// بررسی لود شدن کتابخانه‌های مورد نیاز
function checkDependencies() {
  if (!window.ethers) {
    console.error("Ethers.js not loaded");
    document.getElementById("message").innerText = "خطا: کتابخانه ethers.js لود نشد.";
    return false;
  }
  if (walletProvider === "walletconnect" && !window.WalletConnectProvider) {
    console.error("WalletConnect SDK not loaded");
    document.getElementById("message").innerText = translations[currentLanguage].walletNotDetected;
    return false;
  }
  if (walletProvider === "coinbase" && !window.CoinbaseWalletSDK) {
    console.error("Coinbase Wallet SDK not loaded");
    document.getElementById("message").innerText = translations[currentLanguage].walletNotDetected;
    return false;
  }
  return true;
...
function updateTexts() {
  ...
  document.querySelector('.miner-status').textContent = t.minerStatus + ': ';
  document.querySelector('#miner-rewards').previousSibling.textContent = t.minerRewards + ': ';
  document.getElementById("buy-miner").textContent = t.buyMiner;
  document.getElementById("withdraw-pool-btn").textContent = t.withdrawPoolBtn;
  document.getElementById("withdraw-special-btn").textContent = t.withdrawSpecialBtn;
}
...
function selectWalletProvider() {
  ...
  console.log("Selected wallet provider:", walletProvider);
}
  ...
  try {
    ...
    if (switchError.code === 4902) {
      try {
        ...
        console.log("Polygon Mainnet added");
        return true;
      } catch (addError) {
        document.getElementById("message").innerText = translations[currentLanguage].errorConnection + addError.message;
        console.error("Add network error:", addError);
        return false;
      }
    } else {
      document.getElementById("message").innerText = translations[currentLanguage].switchNetwork;
      console.error("Switch network failed:", switchError.message);
      return false;
    }
  }
}
...
async function connectWallet() {
  ...
  console.log("Attempting to connect with provider:", walletProvider);

  try {
    // قطع اتصال قبلی برای WalletConnect
    ...
    if (walletProvider === "metamask") {
      ...
      try {
        ...
        document.getElementById("message").innerText = translations[currentLanguage].walletConnectFailed;
        console.error("WalletConnect enable failed:", err);
        return;
      }
      ...
      provider = new ethers.providers.Web3Provider(coinbaseWallet.makeWeb3Provider("https://polygon-rpc.com", 137));
      await provider.send("eth_requestAccounts", []);
      console.log("Coinbase Wallet accounts requested");
      const network = await provider.getNetwork();
      if (network.chainId !== 137) {
        if (!(await switchToPolygon())) return;
      }
    } else {
      document.getElementById("message").innerText = translations[currentLanguage].installWallet;
      console.error("Invalid wallet provider:", walletProvider);
      return;
    }
    ...
    document.getElementById("message").innerText = translations[currentLanguage].errorConnection + (err.message || "اتصال ناموفق بود");
...
async function fetchMinerInfo() {
  if (contract && userAccount) {
    try {
      const user = await contract.getUserInfo(userAccount);
      document.getElementById("miner-status").innerText = user.isMiner ? translations[currentLanguage].minerStatus + ": فعال" : translations[currentLanguage].minerStatus + ": غیرفعال";
      document.getElementById("miner-rewards").innerText = translations[currentLanguage].minerRewards + ": " + ethers.utils.formatEther(user.totalMinerRewards) + " MATIC";
      console.log("Miner info fetched:", user);
    } catch (err) {
      document.getElementById("message").innerText = translations[currentLanguage].errorUserInfo + err.message;
      console.error("Fetch miner info error:", err);
    }
  }
}
...
async function register() {
  ...
  if (!uplineCode || isNaN(uplineCode)) {
    ...
    console.error("Invalid upline code:", uplineCode);
    return;
  }
  ...
  try {
    ...
    const estimatedGasCost = gasPrice.maxFeePerGas.mul(gasLimit);
    ...
    if (balance.lt(totalRequired)) {
      ...
      console.error("Insufficient balance:", ethers.utils.formatEther(balance));
      return;
    }
...
async function displayTree() {
  ...
  async function getUserById(userId) {
    ...
    try {
      ...
      console.log("Fetched user by ID:", userId, user);
      return user;
    } catch (err) {
  ...
  try {
    ...
    console.error("Display tree error:", err);
  }
}
...
async function withdrawPool() {
  try {
    ...
    console.log("Withdraw pool transaction sent:", tx.hash);
    await tx.wait();
...
async function withdrawSpecials() {
  try {
    ...
    console.log("Withdraw specials transaction sent:", tx.hash);
    await tx.wait();
...
window.addEventListener('load', () => {
  detectWalletApp();
  updateTexts();
  console.log("Page loaded, wallet provider:", walletProvider);
});