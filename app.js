...
async function loadWeb3() {
  return new Promise((resolve, reject) => {
    ...
    script.src = "https://cdnjs.cloudflare.com/ajax/libs/web3/4.3.0/web3.min.js";
script.onload = () => {
      if (typeof window.Web3 !== 'undefined') {
        web3 = new Web3(window.ethereum);
        resolve();
      } else {
        reject(new Error("Web3 لود نشد: مشکل در CDN"));
      }
    };
    script.onerror = () => reject(new Error("Web3 لود نشد: خطا در لود فایل"));
...
async function checkNetwork() {
  ...
  const polygonChainId = '0x89'; // Chain ID 137 در هگزادسیمال
  if (chainId !== polygonChainId) {
    try {
      ...
      document.getElementById("message").innerText = "خطا: لطفاً شبکه را به Polygon Mainnet تغییر دهید";
      return false;
    }
  }
  return true;
...
async function connectWallet() {
  if (window.ethereum) {
    try {
      await loadWeb3(); // صبر کن تا Web3 لود بشه
