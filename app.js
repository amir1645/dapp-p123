...
const translations = {
  ...
  ar: {
    ...
    installWallet: "يرجى تثبيت محفظة (مثل MetaMask) أو استخدام متصفح المحفظة الداخلي!",
  ...
  en: {
    ...
    installWallet: "Please install a wallet (e.g., MetaMask) or use the wallet's built-in browser!",
...
// تابع سوئیچ به شبکه Polygon
async function switchToPolygon() {
  try {
    await window.ethereum.request({
      method: 'wallet_switchEthereumChain',
      params: [{ chainId: '0x89' }], // chainId 137 برای Polygon
    });
...
async function connectWallet() {
  ...
  try {
    ...
    if (walletProvider === "metamask") {
      ...
      console.error("Invalid wallet provider selected:", walletProvider);
      return;
    }
...
function disconnectWallet() {
  ...
  setTimeout(() => {
    ...
    walletProvider = document.getElementById("wallet-provider").value;
    console.log("Wallet disconnected, provider reset to:", walletProvider);
  }, 500);
}
...
async function register() {
  ...
  try {
    const network = await provider.getNetwork();
    console.log("Current network for registration:", network.chainId);
    if (network.chainId !== 137) {
      document.getElementById("message").innerText = translations[currentLanguage].switchNetwork;
