...
const translations = {
  fa: {
    ...
    walletNotDetected: "کیف پول شناسایی نشد. لطفاً مطمئن شوید که افزونه یا اپلیکیشن کیف پول (مثل MetaMask) نصب است.",
  ...
  ar: {
    ...
    walletNotDetected: "لم يتم اكتشاف المحفظة. يرجى التأكد من تثبيت إضافة أو تطبيق المحفظة (مثل MetaMask).",
  ...
  en: {
    ...
    walletNotDetected: "Wallet not detected. Please ensure the wallet extension or app (e.g., MetaMask) is installed.",
...
function openTab(tabId) {
  ...
  if (tabId === 'miner-tab') fetchMinerInfo();
  else if (tabId === 'tree-tab') displayTree();
}
...
async function connectWallet() {
  document.getElementById("message").innerText = "در حال اتصال...";
  ...
  try {
    if (provider && walletProvider === "walletconnect") await provider.provider.disconnect();
    provider = null;
    signer = null;
    contract = null;
    userAccount = null;
    ...
    document.getElementById("message").innerText = "اتصال موفقیت‌آمیز بود!";
    document.getElementById("connect-btn").classList.remove("loading");
  } catch (err) {
...
function disconnectWallet() {
  if (provider && walletProvider === "walletconnect") provider.provider.disconnect();
  provider = null;
  signer = null;
  contract = null;
  userAccount = null;
...
async function register() {
  document.getElementById("message").innerText = "در حال ثبت‌نام...";
...
async function displayTree() {
  ...
  async function buildBinaryTree(userAddress, depth = 0) {
    ...
    try {
      ...
      let treeHTML = `<div class="tree-node" style="animation: slideIn ${0.5 + depth * 0.2}s ease-out;">`;
      ...
      treeHTML += await buildBinaryTree(leftUser.userAddress, depth + 1);
      treeHTML += `</div>`;
      treeHTML += `<div class="branch right">`;
      treeHTML += `<span class="branch-label">سمت راست:</span><br>`;
      treeHTML += await buildBinaryTree(rightUser.userAddress, depth + 1);
      treeHTML += `</div>`;
      treeHTML += `</div>`;
      treeHTML += `</div>`;
      return treeHTML;
...
async function buyMinerTokens() {
  document.getElementById("message").innerText = "در حال خرید توکن‌ها...";
  try {
    if ((await provider.getNetwork()).chainId !== 137) {
      document.getElementById("message").innerText = translations[currentLanguage].switchNetwork;
      await switchToPolygon();
...
async function withdrawPool() {
  document.getElementById("message").innerText = "در حال برداشت...";
  try {
    if ((await provider.getNetwork()).chainId !== 137) {
      document.getElementById("message").innerText = translations[currentLanguage].switchNetwork;
      await switchToPolygon();
